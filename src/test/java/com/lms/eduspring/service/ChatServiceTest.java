...
import com.lms.eduspring.service.ChatServiceImpl;
...
class ChatServiceTest {
    ...
    void setUp() {
        ...
        mockUser = new User("john", "pass", "John", "Doe",
                "john@example.com", "STUDENT", Set.of());
    ...
    void testCreateNewChatSession_WhenNoSessionId() {
        ...
        ChatSession result = chatService.processUserMessage(
                1L, null, "Hello world!"
        );
    ...
    void testGetMessagesForUserSession_WhenUserOwnsSession() {
        ...
        when(chatSessionRepository.findById(5L))
                .thenReturn(Optional.of(session));

        var result = chatService.getMessagesForUserSession(
                1L, 5L
        );
    ...
    void testGetMessagesForUserSession_WhenUserNotOwner_ShouldThrow() {
        ...
        User other = new User("other", "pass", "A", "B",
                "other@example.com", "STUDENT", Set.of());
        ReflectionTestUtils.setField(other, "id", 2L);

        ChatSession session = new ChatSession("Private", other);
        ReflectionTestUtils.setField(session, "id", 9L);

        when(chatSessionRepository.findById(9L))
                .thenReturn(Optional.of(session));
